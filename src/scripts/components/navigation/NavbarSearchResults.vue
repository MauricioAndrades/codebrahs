...
<script>
  ...
  import { allProductsHandle } from 'scripts/lib/config.collections.js'
  ...
  export default {
    ...
    computed: {
      ...
      searchResults () {
        let searchParam = this.search.toLowerCase()
        let filteredProducts = (search.length < 1) 
          ? [] : this.collection.products.filter(product => product.title.toLowerCase().indexOf(searchParam) > -1)
